...
Create a new Drupal 11 content entity for aegir-hosting. This command covers the full entity creation process following project patterns.
...
Path: `{module}/src/Entity/{EntityName}.php`
...
> **Critical**: `admin_permission` and `route_provider` handler are required. Without `route_provider`, `links` in the attribute do nothing.
...
### 5. Add Routing (custom routes only)

With `AdminHtmlRouteProvider`, routes for `canonical`, `add-form`, `edit-form`, `delete-form`, and `collection` are **auto-generated**. Only add custom routes (e.g., task operations):

```yaml
# {module}/{module}.routing.yml
{module}.task.{operation}:
  path: '/hosting/{entity_type_plural}/{entity_id}/{operation}'
  defaults:
    _form: '\Drupal\{module}\Form\{EntityName}{Operation}Form'
    _title: '{Operation}'
  requirements:
...
> Always add `{entity_id}: \d+` to route requirements.

### 6. Add Entity Lifecycle Hooks (Drupal 11 style)

Empty module file `{module}/{module}.module`:
```php
<?php
...
Hook class at `{module}/src/Hook/{EntityName}Hooks.php`:
```php
<?php
...
- ❌ Do NOT omit the `route_provider` handler — without it, `links` do nothing
- ❌ Do NOT use annotation-style docblocks — use PHP 8 `#[ContentEntityType]`
- ❌ Do NOT use procedural `hook_*()` functions — use `#[Hook]` attributes in `src/Hook/` classes
- ❌ Do NOT call `drush entity:updates` — command does not exist in Drush 13; reinstall instead
